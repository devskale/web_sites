...
### Fixed
- **Solar "Heute" bug** — the summary previously showed the *last* forecast day as "Heute" (e.g. it displayed Jun 27's production while today was Jun 24). Now resolves the actual location-local current date via `utc_offset_seconds`.

### Added
- **Sunrise / sunset / daylight** strip under the current weather cards (the missing core data for a solar-focused app). Data via Open-Meteo `daily=sunrise,sunset,daylight_duration`.
- **Geolocation button** — "use my current location" via the browser Geolocation API (next to the city presets).
- **Wind direction** — compass arrow + label (N/NO/O/…) on the wind card; data was fetched but unused before.
